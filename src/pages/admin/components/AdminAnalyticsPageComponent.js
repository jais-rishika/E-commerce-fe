...
export default function AdminAnalyticsPageComponent({
  ...
  useEffect(() => {
    ...
    fetchOrdersForSecondDate(abctrl, secondDateToCompare)
      ...
      .catch((er) =>
        console.log(
          er
          //   .response.data.message ? er.response.data.message : er.response.data
        )
      );
    return () => abctrl.abort();
            ...
            {dataForFirstSet.length > dataForSecondSet.length ? (
              <>
                <Line
                  ...
                  strokeWidth={4}
                />
                <Line type="monotone" dataKey="2024 year" stroke="#82ca9d" />
              </>
            ) : (
              <>
                <Line
